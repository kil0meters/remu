    ...
    fn execute_compressed(&mut self, inst: u16) {
        ...
        match (quadrant, funct3) {
            (0b10, 0b000) => {
                ...
                log::debug!("{:016x} slli  x{}, x{}, {}", self.pc, rd, rd, shift_amount);
            }
            (0b10, 0b100) => {
                let imm = (inst >> 12) & 0b1;
            ...
            (0b10, 0b011) => {
                ...
                if rd != 0 {
                    // C.LDSP
                    log::debug!("{:016x} ld    x{}, {}(sp)", self.pc, rd, imm);
                    self.reg[rd as usize] = self
                        .memory
                        .load_u64((imm as u64).wrapping_add(self.reg[SP]));
                } else {
                    log::error!("C.FLWSP not implemented");
                }
                //
            }
            ...
            (0b01, 0b000) => {
                ...
                log::debug!("{:016x} addi  x{}, x{}, {}", self.pc, rd, rd, imm);
            ...
            (0b01, 0b001) => {
                ...
                log::debug!("{:016x} addiw x{}, x{}, {}", self.pc, rd, rd, imm);
            ...
            (0b01, 0b010) => {
                ...
                log::debug!("{:016x} li    x{}, {}", self.pc, rd, imm);
            ...
            (0b01, 0b100) => {
                ...
                match funct2 {
                    ...
                    0b00 => {
                        ...
                        log::debug!("{:016x} srli  x{}, x{}, {}", self.pc, rd, rd, imm);
                    ...
                    0b10 => {
                        ...
                        log::debug!("{:016x} andi  x{}, x{}, {}", self.pc, rd, rd, imm);
                    ...
                    0b11 => {
                        ...
                        match (imm_bit, funct2_2) {
                            ...
                            (0, 0b00) => {
                                ...
                                log::debug!("{:016x} sub   x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }

                            // C.XOR
                            (0, 0b01) => {
                                self.reg[rd] = self.reg[rd] ^ self.reg[rs2];
                                log::debug!("{:016x} xor   x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }

                            // C.OR
                            (0, 0b10) => {
                                self.reg[rd] = self.reg[rd] | self.reg[rs2];
                                log::debug!("{:016x} or    x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }

                            // C.AND
                            (0, 0b11) => {
                                self.reg[rd] = self.reg[rd] & self.reg[rs2];
                                log::debug!("{:016x} and   x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }
                            ...
                            (1, 0b00) => {
                                ...
                                log::debug!("{:016x} subw  x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }
                            ...
                            (1, 0b01) => {
                                self.reg[rd] =
                                    (self.reg[rd] as i32).wrapping_sub(self.reg[rs2] as i32) as u64;
                                log::debug!("{:016x} addw  x{}, x{}, x{}", self.pc, rd, rd, rs2);
                            }
            ...
            (0b01, 0b101) => {
                ...
                log::debug!("{:016x} j     {:x}", self.pc, addr);
            ...
            (0b01, 0b110) => {
                ...
                log::debug!("{:016x} bnez  x{}, {:x}", self.pc, rs1, addr);
            ...
            (0b00, 0b000) => {
                ...
                log::debug!("{:016x} addi  x{}, sp, {}", self.pc, rd, imm);
            ...
            (0b00, 0b010) => {
                ...
                log::debug!("{:016x} lw    x{}, {}(x{}) (c.ld)", self.pc, rd, imm, rs1);
            ...
            (0b00, 0b011) => {
                ...
                log::debug!("{:016x} ld    x{}, {}(x{}) (c.ld)", self.pc, rd, imm, rs1);
            ...
            (0b00, 0b110) => {
                ...
                log::debug!("{:016x} sd    x{}, {}(x{}) (c.sw)", self.pc, rs2, imm, rs1);
            ...
            (0b00, 0b111) => {
                ...
                log::debug!("{:016x} sd    x{}, {}(x{}) (c.sd)", self.pc, rs2, imm, rs1);
    ...
    fn execute_general(&mut self, inst: u32) {
        ...
        match opcode {
            ...
            0b0000011 => {
                ...
                log::debug!("{} + {} = {}", self.reg[rs1], offset, addr);

                match funct3 {
                    // LW
                    0b010 => {
                        log::debug!("{:016x} lw    x{}, {}(x{})", self.pc, rd, offset, rs1);
                        self.reg[rd] = self.memory.load_u32(addr) as i32 as u64;
                    }
                    // LD
                    0b011 => {
                        log::debug!("{:016x} ld    x{}, {}(x{})", self.pc, rd, offset, rs1);
                        self.reg[rd] = self.memory.load_u64(addr) as u64;
                    }

                    // LBU
                    0b100 => {
                        log::debug!("{:016x} lbu   x{}, {}(x{})", self.pc, rd, offset, rs1);
                        self.reg[rd] = self.memory.load_byte(addr) as i8 as u64;
                    }

                    0b101 => {
                        log::debug!("{:016x} lhu   x{}, {}(x{})", self.pc, rd, offset, rs1);
            ...
            0b0010011 => {
                ...
                match funct3 {
                    // ADDI
                    0b000 => {
                        log::debug!("{:016x} addi  x{}, x{}, {}", self.pc, rd, rs1, imm);
                    ...
                    0b001 => {
                        ...
                        log::debug!("{:016x} slli  x{}, x{}, {}", self.pc, rd, rs1, imm);
                        self.reg[rd] = self.reg[rs1] << shamt;
                    }
                    0b111 => {
                        log::debug!("{:016x} andi  x{}, x{}, {}", self.pc, rd, rs1, imm);
            ...
            0b0010111 => {
                ...
                log::debug!("{:016x} auipc x{}, 0x{:x}", self.pc, rd, imm);
            ...
            0b0011011 => {
                ...
                match funct3 {
                    ...
                    0b000 => {
                        ...
                        log::debug!("{:016x} addiw x{}, x{}, {}", self.pc, rd, rs1, imm as i32);
                    ...
                    0b001 => {
                        ...
                        log::debug!("{:016x} slliw x{}, x{}, {}", self.pc, rd, rs1, shamt);
            ...
            0b0100011 => {
                ...
                match funct3 {
                    // SD
                    0b011 => {
                        log::debug!("{:016x} sd    x{}, {}(x{})", self.pc, rs2, imm, rs1);
                        self.memory.store_u64(addr, self.reg[rs2]);
                    }
                    // SB
                    0b000 => {
                        log::debug!("{:016x} sb    x{}, {}(x{})", self.pc, rs2, imm, rs1);
                    ...
                    // SW
                    0b010 => {
                        log::debug!("{:016x} sw    x{}, {}(x{})", self.pc, rs2, imm, rs1);
            ...
            // ADD
            0b0110011 => {
                log::debug!("{:016x} add   x{}, x{}, x{}", self.pc, rd, rs1, rs2);
            ...
            0b1100011 => {
                ...
                match funct3 {
                    // BEQ
                    0b000 => {
                        log::debug!("{:016x} beq   x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = self.reg[rs1] == self.reg[rs2];
                    }
                    // BNE
                    0b001 => {
                        log::debug!("{:016x} bne   x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = self.reg[rs1] != self.reg[rs2];
                    }
                    // BLT
                    0b100 => {
                        log::debug!("{:016x} blt   x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = (self.reg[rs1] as i64) < self.reg[rs2] as i64;
                    }
                    // BGE
                    0b101 => {
                        log::debug!("{:016x} bge   x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = (self.reg[rs1] as i64) >= self.reg[rs2] as i64;
                    }
                    // BLTU
                    0b110 => {
                        log::debug!("{:016x} bltu  x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = self.reg[rs1] < self.reg[rs2];
                    }
                    // BGEU
                    0b111 => {
                        log::debug!("{:016x} bgeu  x{}, x{}, {:x}", self.pc, rs1, rs2, addr);
                        condition = self.reg[rs1] >= self.reg[rs2];
                    }
            ...
            0b1101111 => {
                ...
                log::debug!("{:016x} jal   x{}, {:x}", self.pc, rd, addr);
            ...
            _ => {
                log::error!("{:016x} opcode `{opcode:07b}` not implemented.", self.pc);
                panic!();
            }
        }
    }
